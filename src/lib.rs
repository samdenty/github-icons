        ...
        if let Ok(icons) = icons {
            let mut tasks: Vec<_> = icons
                ...
                .map(|(i, icon)| -> JoinHandle<Option<()>> {
                    ...
                    tokio::spawn(async move {
                        let cache_name =
                            format!("{}{}", icon.url.host_str().unwrap_or(""), icon.url.path())
                                .replace("/", "-")
                                .replace(":", "-");
                            ...
                            match icon.url.scheme() {
                                "data" => {
                                    ...
                                    if let Some(type_index) = type_index {
                                        let data_type =
                                            data_uri_path[(type_index + 1)..data_index].to_string();
                                        if data_type == "base64" {
                                            let mut content = Cursor::new(
                                                base64::decode(&data).unwrap_or(Vec::new()),
                                            );
                                    ...
                                    if !written {
                                        let mut content = Cursor::new(urlencoding::decode_binary(
                                            &data.as_bytes(),
                                        ));
                                        copy(&mut content, &mut icon_file).await.ok()?;
                                    }
    ...
    pub async fn set_default(slug_or_path: &str) -> Result<(), Box<dyn Error>> {
...

        {
            ...
            if let Some(repo_path) = repo_path {
                ...
                diesel::delete(repos.filter(owner.eq(&user).and(repo.eq(&repo_name))))
                    .execute(db())?;
            };
        }
            ...
            if let Some(icon_path) = repo.icon_path {
                ...
                if !icon_rsrc.exists() {
                    ...
                    let img = if icon_path.extension().unwrap() == "svg" {
                        ...
                        let (width, height) =
                            resize_box(1024, 1024, pixmap_size.width(), pixmap_size.height());
                    ...
                    let mut img =
                        ImageBuffer::from_fn(1024, 1024, |_x, _y| image::Rgba([255, 255, 255, 1]));
